...
# Define the custom MSCOCO dataloader
class MSCOCODataLoader:
    ...
    def collate_fn(self, batch):
        # The dataset items have keys "jpg" (image) and "txt" (captions)
        images = [item["jpg"] for item in batch]
        # Split captions on newline in case there are multiple captions per image
...
data_loader_obj = MSCOCODataLoader(split="test", batch_size=32)
...
# Custom DataLoader for MSCOCO Captions for Bias evals (only one caption per image)
class MSCOCO1CDataLoader:
    def __init__(self, split="test", batch_size=32):
        self.split = split
        self.batch_size = batch_size
        self.dataset = None
    ...
    def collate_fn(self, batch):
        # In the dataset, images are under key "jpg" and captions under "txt".
        # Here we take only the first caption per image for evaluation.
        images = [item["jpg"] for item in batch]
        captions = [item["txt"].split('\n')[0] for item in batch]
        return {"images": images, "captions": captions}
...
data_loader_obj = MSCOCO1CDataLoader(split="test", batch_size=32)
dataloader = data_loader_obj.get_dataloader()
